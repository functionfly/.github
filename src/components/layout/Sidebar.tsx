...
function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const handleDrop = (e: React.DragEvent, targetSectionId: string) => {
    ...
    if (draggingSection && draggingSection !== targetSectionId) {
      // Reorder sections (simplified - would need proper implementation)
  ...
  const OnboardingProgress = () => {
    if (!showOnboardingHints || isOnboardingComplete) return null;

    return (
      <div className="aviation-onboarding-progress">
        <div className="flex items-center justify-between text-xs">
          <span className="text-aviation-text-secondary">Getting Started</span>
          <span className="text-aviation-amber font-medium">{onboardingProgress}%</span>
        </div>
  ...
  const NavItemComponent = ({
    ...
    const showOnboardingHint =
      showOnboardingHints &&
      item.onboardingHint &&
      !completedOnboardingSteps.includes(sectionId);
      ...
      <Tooltip key={item.path}>
        <TooltipTrigger asChild>
          <NavLink
            ...
            onClick={() => {
              onClose();
              if (item.onboardingHint) {
                // Mark onboarding step as seen
              }
            }}
            className={cn(
  ...
  const SectionHeader = ({
    ...
    return (
      ...
      >
        ...
        {!isCollapsed && isLg && section.collapsible && (
          <button
      ...
      <>
        ...
        >
          ...
          <div className="p-3 border-t border-aviation-border-panel">
            ...
            {/* Sign Out */}
            <button className="aviation-signout" onClick={handleLogout}>
