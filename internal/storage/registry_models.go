...
// BlockedPattern represents patterns that have been administratively blocked
type BlockedPattern struct {
	ID          uuid.UUID  `json:"id" gorm:"type:uuid;primaryKey;default:gen_random_uuid()"`
	PatternType string     `json:"pattern_type" gorm:"size:50;index"` // "bot_pattern", "ip_cluster", "wash_trading", etc.
	PatternID   string     `json:"pattern_id" gorm:"size:100;index"`  // ID of the specific pattern instance
	Description string     `json:"description" gorm:"type:text"`
	BlockedBy   uuid.UUID  `json:"blocked_by" gorm:"type:uuid"` // Admin user who blocked it
...
// Investigation represents an administrative investigation of suspicious activity
type Investigation struct {
	ID         uuid.UUID  `json:"id" gorm:"type:uuid;primaryKey;default:gen_random_uuid()"`
	EntityType string     `json:"entity_type" gorm:"size:50"`            // "function", "user", "tenant", "ip", "pattern"
	EntityID   string     `json:"entity_id" gorm:"size:100"`             // ID of the entity being investigated
	Reason     string     `json:"reason" gorm:"type:text"`               // Reason for investigation
	OpenedBy   uuid.UUID  `json:"opened_by" gorm:"type:uuid"`            // Admin who opened investigation
	Status     string     `json:"status" gorm:"size:50"`                 // "open", "closed", "escalated"
	Priority   string     `json:"priority" gorm:"size:20"`               // "low", "medium", "high", "critical"
	Findings   string     `json:"findings,omitempty" gorm:"type:text"`   // Investigation findings
	Resolution string     `json:"resolution,omitempty" gorm:"type:text"` // How it was resolved
