...
// DatabaseEncryptionManager interface for encrypting/decrypting fields
type DatabaseEncryptionManager interface {
...
// NewRepositoryWithEncryption creates a new DNA repository with encryption support.
func NewRepositoryWithEncryption(db *sql.DB, enc DatabaseEncryptionManager) *Repository {
	return &Repository{db: db, encryptionManager: enc}
}

// encryptCode encrypts code if encryption is enabled and not already encrypted
func (r *Repository) encryptCode(code string) (string, error) {
	if r.encryptionManager == nil || !r.encryptionManager.IsEncryptionEnabled() {
		return code, nil
	}
	if code == "" {
		return code, nil
	...
	return r.encryptionManager.EncryptField(code)
}

// decryptCode decrypts code if encryption is enabled and is encrypted
func (r *Repository) decryptCode(code string) (string, error) {
	if r.encryptionManager == nil || !r.encryptionManager.IsEncryptionEnabled() {
		return code, nil
	}
	if code == "" || !strings.HasPrefix(code, "ENC:") {
		return code, nil
	}
	return r.encryptionManager.DecryptField(code)
}
...
func (r *Repository) ListMutations(ctx context.Context, functionID, tenantID, status string, limit, offset int) ([]*Mutation, int, error) {
	...
	for rows.Next() {
		...
		var origCode, mutCode sql.NullString
		if err := rows.Scan(
		...
		if origCode.Valid {
			decrypted, err := r.decryptCode(origCode.String)
			if err != nil {
				return nil, 0, fmt.Errorf("decrypt original code: %w", err)
			}
			m.OriginalCode = &decrypted
		}
		if mutCode.Valid {
			decrypted, err := r.decryptCode(mutCode.String)
			if err != nil {
				return nil, 0, fmt.Errorf("decrypt mutated code: %w", err)
			}
			m.MutatedCode = &decrypted
		}
...
// AcceptMutationTx atomically accepts a mutation using a row-level lock to prevent
// race conditions where two callers attempt to accept the same mutation concurrently.
func (r *Repository) AcceptMutationTx(ctx context.Context, tx *sql.Tx, mutationID, userID string) (*Mutation, error) {
	...
	err := tx.QueryRowContext(ctx, `
		...
		RETURNING id, function_id, function_type, tenant_id, generation, mutation_type, status,
		          accepted_by, accepted_at, created_at`,
		mutationID, userID,
	).Scan(
		&m.ID, &m.FunctionID, &m.FunctionType, &m.TenantID, &m.Generation, &m.MutationType, &m.Status,
		&acceptedBy, &acceptedAt, &m.CreatedAt,
	)
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("mutation %s not found or not in pending state", mutationID)
	}
	if err != nil {
		return nil, fmt.Errorf("accept mutation: %w", err)
	}
	m.AcceptedBy = acceptedBy
...
func (r *Repository) GetMutation(ctx context.Context, mutationID string) (*Mutation, error) {
	...
	`, mutationID).Scan(
		&m.ID, &m.FunctionID, &m.FunctionType, &m.TenantID, &m.Generation,
		&m.MutationType, &m.Status, &m.TriggerReason,
		&origCode, &mutCode, &m.OriginalHash, &m.MutatedHash,
	...
	if origCode.Valid {
		decrypted, err := r.decryptCode(origCode.String)
		if err != nil {
			return nil, fmt.Errorf("decrypt original code: %w", err)
		}
		m.OriginalCode = &decrypted
	}
	if mutCode.Valid {
		decrypted, err := r.decryptCode(mutCode.String)
		if err != nil {
			return nil, fmt.Errorf("decrypt mutated code: %w", err)
		}
		m.MutatedCode = &decrypted
	}
	if actualImpact.Valid {
...
func (r *Repository) CreateMutation(ctx context.Context, m *Mutation) error {
	...
	var origCodeVal, mutCodeVal *string
	if m.OriginalCode != nil {
		encrypted, err := r.encryptCode(*m.OriginalCode)
		if err != nil {
			return fmt.Errorf("encrypt original code: %w", err)
		}
		origCodeVal = &encrypted
	}
	if m.MutatedCode != nil {
		encrypted, err := r.encryptCode(*m.MutatedCode)
		if err != nil {
			return fmt.Errorf("encrypt mutated code: %w", err)
		}
		mutCodeVal = &encrypted
	}
	...
	`, m.ID, m.FunctionID, m.FunctionType, m.TenantID, m.Generation,
		m.MutationType, m.Status, m.TriggerReason,
		origCodeVal, mutCodeVal, m.OriginalHash, m.MutatedHash,
...
// GetQueueDepth returns the number of mutations currently waiting in the payment reconciliation queue.
func (r *Repository) GetQueueDepth(ctx context.Context) (int, error) {
	var depth int
	err := r.db.QueryRowContext(ctx, `
		SELECT COUNT(*)
		FROM function_dna_mutations
