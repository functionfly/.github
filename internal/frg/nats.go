...
type NATSEventStream struct {
	...
	streams     map[uuid.UUID]nats.JetStreamContext
...
func NewNATSEventStream(config *NATSConfig) (*NATSEventStream, error) {
	...
	stream := &NATSEventStream{
		nc:           nc,
		js:           js,
		streams:      make(map[uuid.UUID]nats.JetStreamContext),
...
func (s *NATSEventStream) Publish(event *GraphEvent) error {
	s.mu.RLock()
	js, ok := s.streams[event.InstanceID]
	s.mu.RUnlock()
...
func (s *NATSEventStream) createStream(instanceID uuid.UUID) error {
	...
	// Get stream context
	stream, err := s.js.Stream(streamName)
	if err != nil {
...
// InMemoryEventStream is a simple in-memory implementation for testing
type InMemoryEventStream struct {
	subscribers map[uuid.UUID][]func(*GraphEvent)
	mu          sync.RWMutex
}

// NewInMemoryEventStream creates an in-memory event stream for testing
