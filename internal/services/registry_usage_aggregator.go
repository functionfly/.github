...
func (a *RegistryUsageAggregator) aggregateExecutions(ctx context.Context, start, end time.Time) ([]*AggregatedUsage, error) {
	...
	for _, result := range results {
		usage := &AggregatedUsage{
			...
			PeriodStart:    start,
			PeriodEnd:      end,
			TotalCalls:     int64(result.TotalCalls),
			SuccessCalls:   int64(result.SuccessCalls),
			ErrorCalls:     int64(result.ErrorCalls),
			CachedCalls:    int64(result.CachedCalls),
			TotalDuration:  result.TotalDuration,
			AvgDuration:    int64(result.AvgDuration),
