      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div
              className={`p-3 rounded-full ${status?.enabled ? 'bg-yellow-100' : 'bg-green-100'}`}
            >
              <AlertTriangle
                className={`w-6 h-6 ${status?.enabled ? 'text-yellow-600' : 'text-green-600'}`}
              />
            </div>
            <div>
      ...
      {activeTab === 'status' && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          {statusLoading ? (
            ...
            <div className="p-8 text-center text-gray-500">No scheduled maintenance windows</div>
          ) : (
      ...
      {activeTab === 'schedule' && (
        <div className="bg-white rounded-lg shadow p-6">
          <form className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Window Name</label>
              <input
                type="text"
      ...
      {activeTab === 'templates' && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          {status?.templates?.length === 0 ? (
            <div className="p-8 text-center text-gray-500">No maintenance templates configured</div>
          ) : (
