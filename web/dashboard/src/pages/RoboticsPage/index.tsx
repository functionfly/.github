/**
 * Robotics Page
 * Unified workspace for robotics fleet management and physical systems control
 */
...
// Navigation items for robotics views
const VIEW_NAV_ITEMS = [
  { id: 'fleet', label: 'Fleet', icon: Bot },
  { id: 'sensor', label: 'Telemetry', icon: Radio },
  { id: 'command', label: 'Command', icon: Crosshair },
  { id: 'map', label: 'Map', icon: Map },
  { id: 'drone', label: 'Drone', icon: Navigation },
  { id: 'vision', label: 'Vision', icon: Eye },
  { id: 'mesh', label: 'Mesh', icon: Network },
  { id: 'actuator', label: 'Actuator', icon: Cpu },
...
const generateMockSensorReadings = (robotId: string): SensorReading[] => [
...
const generateMockCommands = (robotId: string): Command[] => [
...
const generateMockVisionFrames = (robotId: string): VisionFrame[] => [
  {
    id: 'vf1',
...
const generateMockActuators = (robotId: string): Actuator[] => [
...
export function RoboticsPage() {
  ...
  const mockFleet = generateMockFleet(robots)
  const mockReadings = generateMockSensorReadings(selectedRobotId || 'r1')
  const mockCommands = generateMockCommands(selectedRobotId || 'r1')
  const mockWaypoints = generateMockWaypoints()
  const mockObstacles = generateMockObstacles()
  const mockFlightPaths = generateMockFlightPaths()
  const mockVisionFrames = generateMockVisionFrames(selectedRobotId || 'r1')
  const mockMeshNodes = generateMockMeshNodes()
  const mockActuators = generateMockActuators(selectedRobotId || 'r1')
  const mockEdgeDevices = generateMockEdgeDevices()
  ...
  return (
    <div className={cn('flex h-full bg-aviation-bg-panel rounded-lg border border-aviation-border-panel overflow-hidden')}>
      {/* Navigation Sidebar */}
      <div className={cn(
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-aviation-border-panel bg-aviation-bg-secondary">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Robotics Control Center</span>
            {alerts.length > 0 && (
              <div className="flex items-center gap-1.5 px-2 py-1 bg-red-500/20 rounded border border-red-500/50">
                <AlertTriangle className="w-3 h-3 text-red-400" />
                <span className="text-xs text-red-400">{alerts.length} Alert{alerts.length > 1 ? 's' : ''}</span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2 text-xs text-aviation-text-muted">
            <Gauge className="w-4 h-4" />
            <span>{VIEW_NAV_ITEMS.find(i => i.id === activeView)?.label}</span>
            {selectedRobot && (
              <>
