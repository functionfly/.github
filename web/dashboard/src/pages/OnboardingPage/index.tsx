...
import { Zap, Cloud, Rocket, Shield, Users, CheckCircle, ArrowRight, ArrowLeft, Loader2, HelpCircle, Lightbulb, AlertTriangle } from "lucide-react";
...
export function OnboardingPage() {
  ...
  const { currentStep, completedSteps, completeStep, skipOnboarding, userRole } = useOnboardingStore();
  ...
  const handleBack = () => {
    if (currentStepIndex > 0) { // Allow going back to welcome step
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-text-primary mb-2">
                      ...
                      {completedSteps.includes("welcome") && currentStepIndex === 1 && "Ready to Connect"}
                      {completedSteps.includes("connect-provider") && currentStepIndex === 2 && "Great! Provider Connected"}
                      {completedSteps.includes("deploy-function") && currentStepIndex === 3 && "Function Deployed Successfully"}
                    ...
                    <div className="text-sm text-text-secondary space-y-2">
                      {currentStepIndex === 0 && (
                        <div className="space-y-2">
                          <p>
                            Take a moment to learn about FunctionFly's multi-provider deployment and automatic failover features.
                          </p>
                          {userRole && (
                            <div className={`p-2 rounded text-xs ${
                              userRole === 'admin' ? 'bg-purple-500/10 border border-purple-500/20 text-purple-400' :
                              userRole === 'member' ? 'bg-blue-500/10 border border-blue-500/20 text-blue-400' :
                              'bg-green-500/10 border border-green-500/20 text-green-400'
                            }`}>
                              👋 Welcome {userRole === 'admin' ? 'Team Administrator' : userRole === 'member' ? 'Team Member' : 'Viewer'}!
                              {userRole === 'admin' && ' You can manage providers, deploy functions, and invite team members.'}
                              {userRole === 'member' && ' You can deploy functions and access shared resources.'}
                              {userRole === 'viewer' && ' You have read-only access to team functions and metrics.'}
                            </div>
                          )}
                        </div>
                      )}
                      {completedSteps.includes("welcome") && currentStepIndex === 1 && (
                        <p>
                          You're about to connect your first cloud provider. This allows FunctionFly to deploy functions
                          across multiple providers for high availability.
                        </p>
                      )}
                      {completedSteps.includes("connect-provider") && currentStepIndex === 2 && (
                        <div className="space-y-2">
                          <p>
                            Excellent! Your provider is connected. Now let's deploy your first function to see FunctionFly in action.
                          </p>
                      ...
                      {completedSteps.includes("deploy-function") && currentStepIndex === 3 && (
                        <div className="space-y-2">
                          <p>
                            Your function is live! The final step tests your failover setup to ensure high availability.
                          </p>
                          <div className="bg-blue-500/10 border border-blue-500/20 rounded p-2">
                            <p className="text-blue-400 text-xs">
                              💡 FunctionFly automatically routes traffic to healthy providers if one fails
                            </p>
                          </div>
                        </div>
                      )}
                      {completedSteps.includes("test-failover") && userRole === 'admin' && currentStepIndex === 4 && (
                        <div className="space-y-2">
                          <p>
                            Great! Your failover setup is working. Now let's set up your team for collaboration.
                          </p>
                          <div className="bg-blue-500/10 border border-blue-500/20 rounded p-2">
                            <p className="text-blue-400 text-xs">
                              👥 Invite team members to collaborate on functions and share provider access
                            </p>
                          </div>
                        </div>
                      )}
                      {completedSteps.includes("test-failover") && userRole !== 'admin' && currentStepIndex === 3 && (
                        <div className="space-y-2">
                          <p>
                            Excellent! Your FunctionFly setup is complete and ready for production.
                          </p>
                          <div className="bg-green-500/10 border border-green-500/20 rounded p-2">
                            <p className="text-green-400 text-xs">
                              🎉 You're all set to start deploying functions with high availability!
                            </p>
                          </div>
                        </div>
                      )}
                      {(completedSteps.length === steps.length) && (
                        <div className="space-y-2">
                          <p>
                            Congratulations! Your FunctionFly setup is complete and production-ready.
                          </p>
                          <div className="bg-purple-500/10 border border-purple-500/20 rounded p-2">
                            <p className="text-purple-400 text-xs">
                              🚀 Your functions are now deployed across multiple providers with automatic failover
                            </p>
                          </div>
                        </div>
                      )}
            ...
            <DialogDescription className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span>Basic functions can still be deployed, but without multi-provider benefits</span>
                </div>
              </div>
            </DialogDescription>
          </DialogHeader>
      ...
      {isCompleting && (
        <>
          ...
          >
            ...
            <motion.div
              className="w-24 h-24 bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl"
              animate={{
                rotate: [0, 10, -10, 0],
                scale: [1, 1.1, 1]
              }}
