...
import { TrendingUp, AlertCircle, CheckCircle, BarChart3, AlertTriangle } from 'lucide-react';
...
function EnterpriseSLAPage() {
        ...
        <div className="enterprise-sla-page">
          <Card className="sla-gate-card">
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h2 className="sla-gate-title text-xl font-semibold mb-2">
                Enterprise Feature
              </h2>
  ...
  const isForbidden = overviewError && (overviewError as { response?: { status?: number } })?.response?.status === 403;
    ...
    return (
      <PageLayout title="SLA Dashboard">
        <div className="flex items-center justify-center py-16">
          <LoadingSpinner size="lg" text="Loading SLA data..." />
        </div>
      </PageLayout>
    );
  }
  ...
  const currentUptime = overview?.current_uptime_percent ?? 99.99;
  const slaTarget = overview?.sla_target_percent ?? 99.99;
  const incidentCount = overview?.incident_count ?? 0;
  const uptimeStatus: 'success' | 'warning' | 'error' =
    currentUptime >= slaTarget ? 'success' : currentUptime >= 99 ? 'warning' : 'error';
  const incidentStatus: 'success' | 'warning' | 'error' =
    incidentCount === 0 ? 'success' : incidentCount <= 2 ? 'warning' : 'error';
            ...
            <CardContent>
              {uptimeHistoryLoading && !chartData.length ? (
                ...
                <LineChart
                  data={chartData}
                  series={[
                    { key: 'uptime', name: 'Uptime %', color: 'var(--chart-green, #22c55e)', strokeWidth: 2 },
                  ]}
                  xAxisKey="name"
                  height={280}
                  showLegend={true}
            ...
            <CardContent>
              {incidentsError ? (
                <div className="flex items-center gap-3 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
                  ...
                  <p className="text-sm text-text-secondary">Failed to load incidents. Try again later.</p>
                </div>
...
function SLACard({ title, value, subtitle, icon: Icon, status, index = 0 }: SLACardProps) {
  const iconWrapClass = status === 'success' ? 'success' : status === 'warning' ? 'warning' : 'error';
