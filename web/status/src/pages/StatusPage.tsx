import { Chamber, CornerBrace, GaugeStrip, StatusPill, TrustSeal, AnnotationTag, FrameButton, SealedButton, PageGrid, ReducedMotionGate } from '@/components/containment';
...
import { ArrowRight, Shield, AlertTriangle, Clock, CheckCircle, Activity } from 'lucide-react';
...
export default function StatusPage() {
  ...
  const operationalCount = components.filter((s) => s.status === 'operational').length;
        ...
        <main style={{ paddingTop: '120px', paddingBottom: 'var(--space-8)' }}>
          ...
          >
            ...
            <Chamber ribs>
              ...
              <div className="flex flex-col items-center text-center" style={{ gap: 'var(--space-5)' }}>
                ...
                >
                  {isLoadingStatus ? 'Checking...' : overallStatusLabel(overallStatus)}
                </h1>

                <StatusPill status={mapStatus(overallStatus)} label={statusLabel(overallStatus)} />

                <p style={{ fontSize: '15px', color: 'var(--text-dim)', lineHeight: 1.6, maxWidth: '560px' }}>
                  Real-time monitoring of all FunctionFly services and infrastructure providers.
                </p>
            ...
            <Chamber nested>
              <GaugeStrip
                items={[
                  { value: `${operationalCount}/${components.length}`, label: 'Services Online' },
                  { value: probeLatencyMs != null ? `${Math.round(probeLatencyMs)}` : '—', label: 'Avg Latency (ms)' },
            ...
            <section>
              <div className="flex items-center justify-between" style={{ marginBottom: 'var(--space-5)' }}>
                ...
                >
                  <Shield style={{ width: 20, height: 20, color: 'var(--foil-a)' }} />
                  Service Components
                </h2>
                ...
                {isLoadingComponents
                  ? Array.from({ length: 8 }).map((_, i) => (
                      <Chamber key={i} nested>
                        <div className="animate-pulse" style={{ height: 80, background: 'var(--panel)', borderRadius: 'var(--radius)' }} />
                      </Chamber>
                    ))
                  : components.map((component) => (
                      <Chamber key={component.id} nested style={{ padding: 'var(--space-6)' }}>
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <div style={{ fontSize: '16px', fontWeight: 500, color: 'var(--text)' }}>
                              {component.name}
                            </div>
                            <div style={{ fontSize: '13px', color: 'var(--text-dim)', marginTop: 'var(--space-1)' }}>
                              {component.response_time_ms}ms
                            </div>
                          </div>
                          <StatusPill status={mapStatus(component.status)} label={statusLabel(component.status)} />
                        </div>
                        <div
                          style={{
                        ...
                        >
                          <div
                            style={{
                              height: '100%',
                              width: `${component.uptime_30d || 99.97}%`,
                              borderRadius: '2px',
                              background: component.uptime_30d >= 99.95 ? 'var(--status-ok)' : component.uptime_30d >= 99 ? 'var(--status-pending)' : 'var(--status-revoked)',
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Chamber>
                ...
                >
                  <Activity style={{ width: 18, height: 18, color: 'var(--foil-a)' }} />
                  Uptime Overview
                </h3>
              {isLoading ? (
                ...
                <div className="flex items-center gap-4" style={{ position: 'relative', zIndex: 1 }}>
                    <div style={{ flex: 1 }}>
                          ...
                          return (
                            <div
                              ...
                              style={{
                                minWidth: '4px',
                                borderRadius: '2px',
                                background: hasIssue ? 'var(--status-pending)' : 'var(--status-ok)',
                      ...
                      <div className="flex justify-between" style={{ marginTop: 'var(--space-2)' }}>
                        <span style={{ fontSize: '11px', color: 'var(--text-faint)', fontFamily: 'var(--font-mono)' }}>30 days ago</span>
                        <span style={{ fontSize: '11px', color: 'var(--text-faint)', fontFamily: 'var(--font-mono)' }}>Today</span>
                      </div>
                    </div>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: '26px', fontWeight: 500, color: 'var(--status-ok)' }}>
                      99.97%
                    </div>
                  </div>
                )}
              ...
              <Chamber>
                ...
                <div className="flex items-center justify-between" style={{ marginBottom: 'var(--space-5)' }}>
                  ...
                  >
                    <AlertTriangle style={{ width: 18, height: 18, color: 'var(--status-pending)' }} />
                    Recent Incidents
                  </h3>
                  <Link
                    to="/history"
                ...
                {isLoadingStatus ? (
                  <div className="space-y-3">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <div key={i} className="animate-pulse" style={{ height: 48, background: 'var(--panel-raised)', borderRadius: 'var(--radius)' }} />
                    ))}
                  </div>
                ) : incidents.length === 0 ? (
                  <div className="flex items-center justify-center" style={{ minHeight: 120 }}>
                    <div className="text-center">
                      <CheckCircle style={{ width: 32, height: 32, color: 'var(--status-ok)', margin: '0 auto var(--space-3)' }} />
                  ...
                  <div className="space-y-3">
                    {incidents.slice(0, 5).map((incident) => (
                      ...
                      >
                        ...
                        >
                          <StatusPill status={mapStatus(incident.status === 'resolved' ? 'operational' : incident.status === 'monitoring' ? 'pending' : 'revoked')} label={incident.status} />
                          ...
                          <ArrowRight style={{ width: 14, height: 14, color: 'var(--text-faint)', flexShrink: 0 }} />
                        </div>
                      </Link>
                    ))}
                  </div>
            ...
            {maintenance.length > 0 && (
              <Chamber nested>
                ...
                >
                  <Clock style={{ width: 18, height: 18, color: 'var(--status-pending)' }} />
                  ...
                  {maintenance.map((m) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div style={{ fontSize: '14px', color: 'var(--text)', fontWeight: 500 }}>{m.title}</div>
                        <div style={{ fontSize: '12px', color: 'var(--text-faint)', marginTop: '2px' }}>
            ...
            <Chamber id="subscribe">
              ...
              <div className="text-center" style={{ maxWidth: '480px', margin: '0 auto' }}>
                ...
                <p style={{ fontSize: '15px', color: 'var(--text-dim)', lineHeight: 1.6, marginBottom: 'var(--space-5)' }}>
                  Get notified about incidents and maintenance windows via email or RSS.
                </p>
                <div className="flex gap-3 justify-center flex-wrap">
                  <SealedButton onClick={() => window.open('/api/v1/status/rss', '_blank')}>
                    RSS Feed
                  </SealedButton>
                  <FrameButton onClick={() => trackEvent('status_api_access_clicked')}>
                    API Access
                  </FrameButton>
                </div>
              </div>
