...
def handler(event):
    ...
    try:
        ...
        # Exact match first
        lookup = {v.upper(): k for k,v in CSS_COLORS.items()}
        exact = lookup.get(f"#{h.upper()}")
        if exact:
            return {"ok": True, "result": exact, "exact": True, "hex": f"#{h.upper()}"}
        # Nearest
        best_name, best_dist = None, float("inf")
        for name, chex in CSS_COLORS.items():
            cr,cg,cb = int(chex[1:3],16), int(chex[3:5],16), int(chex[5:7],16)
            d = _color_distance(r,g,b,cr,cg,cb)
            if d < best_dist:
                best_dist, best_name = d, name
        return {"ok": True, "result": best_name, "exact": False, "distance": best_dist}
