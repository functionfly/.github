"""
BIP-39 Mnemonic Generator - Generate cryptographic seed phrases.

Implements BIP-39: https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki
...
BIP39_WORDLIST = [
    ...
    "blame", "blanket", "blast", "bleak", "bless", "blind", "blood", "blossom", "blow", "blue",
    ...
    "buddy", "budget", "buffalo", "build", "bulb", "bulk", "bullet", "bundle", "bunny", "burden",
    ...
    "crop", "cross", "crouch", "crowd", "crucial", "cruel", "cruise", "crunch", "crush", "cry",
    ...
    "dignity", "dilemma", "dinner", "dinosaur", "direct", "dirt", "disagree", "discover", "discuss", "disease",
    ...
    "ginger", "giraffe", "give", "glad", "glance", "glare", "glass", "gleam", "glide", "glimpse",
    "globe", "gloom", "glory", "glove", "glow", "glue", "goat", "goddess", "gold", "gone",
    "good", "goose", "gorilla", "gospel", "gossip", "govern", "gown", "grab", "grace", "grain",
    "grand", "grant", "grape", "grass", "gravity", "great", "green", "grid", "grief", "grit",
    "grocery", "group", "grow", "grunt", "guard", "guess", "guide", "guilt", "guitar", "gum",
    ...
    "hurdle", "hurt", "husband", "hybrid", "ice", "icon", "idea", "identify", "idle", "ignore",
    "ill", "illegal", "illness", "image", "imitate", "immerse", "immune", "impact", "impose", "improve",
    "impulse", "inch", "include", "income", "increase", "index", "indicate", "indoor", "industry", "infant",
    "inflict", "inform", "initial", "inject", "injury", "inmate", "inner", "innocent", "input", "inquiry",
    "insane", "insect", "inside", "inspire", "install", "intact", "interest", "into", "invest", "invite",
    "involve", "iron", "island", "isolate", "issue", "item", "ivory", "jacket", "jaguar", "jail",
    ...
    "loop", "lottery", "loud", "lounge", "love", "loyal", "lucky", "luggage", "lumber", "lunch",
    "luxury", "lyrics", "machine", "mad", "magic", "magnet", "maid", "mail", "main", "major",
    "make", "mammal", "manage", "mandate", "mango", "mansion", "manual", "maple", "marble", "march",
    "margin", "marine", "market", "marriage", "mask", "mass", "master", "match", "material", "matrix",
    "matter", "maximum", "maze", "meadow", "mean", "measure", "media", "melody", "melt", "member",
    "memory", "mention", "menu", "mercy", "merge", "merit", "mesh", "message", "metal", "method",
    "middle", "midnight", "milk", "million", "mimic", "mind", "minimum", "minor", "minute", "miracle",
    "mirror", "misery", "miss", "mistake", "mix", "mixture", "mobile", "model", "modify", "mom",
    ...
    "off", "offer", "office", "often", "olive", "olympic", "omit", "once", "one", "onion",
    "online", "only", "open", "opera", "opinion", "oppose", "option", "orange", "orbit", "orchard",
    "order", "organ", "orient", "original", "orphan", "ostrich", "other", "outdoor", "outer", "output",
    ...
    "raccoon", "race", "rack", "radar", "radio", "raft", "rage", "rail", "rain", "raise",
    ...
    "rib", "ribbon", "rice", "rich", "ride", "ridge", "rifle", "right", "rigid", "ring", "riot",
    "ripple", "risk", "ritual", "rival", "river", "road", "roast", "robot", "robust", "rocket",
    "romance", "roof", "rookie", "room", "rose", "rotate", "rough", "round", "route", "royal",
    "rub", "rubber", "rude", "rug", "rule", "run", "runway", "rural", "sad", "saddle",
    "sadness", "safe", "sail", "salad", "salmon", "salon", "salt", "salute", "same", "sample",
    "sand", "satisfy", "satoshi", "sauce", "sausage", "save", "scale", "scan", "scare", "scatter",
    "scene", "scheme", "school", "science", "scissors", "scorpion", "scout", "scrap", "screen", "script",
    "scrub", "sea", "search", "season", "seat", "second", "secret", "section", "security", "seed",
    "seek", "segment", "select", "sell", "senior", "sense", "sentence", "series", "service", "session",
    ...
    "slogan", "slow", "slush", "small", "smart", "smile", "smoke", "smooth", "snack", "snake",
    "snap", "sniff", "snow", "soap", "soccer", "social", "sock", "soda", "soft", "solar",
    "soldier", "solid", "solution", "solve", "someone", "song", "soon", "sorry", "sort", "soul",
    "sound", "soup", "source", "south", "space", "spare", "spatial", "spawn", "special", "speed",
    "spell", "spend", "sphere", "spider", "spike", "spin", "spirit", "split", "sponsor", "spoon",
    ...
    "unhappy", "uniform", "union", "unique", "unit", "universe", "unknown", "unlock", "until", "unusual",
    "unveil", "update", "upgrade", "uphold", "upon", "upper", "upset", "urban", "usage", "use",
...
WORDLIST_INDEX = {word: idx for idx, word in enumerate(BIP39_WORDLIST)}
ENTROPY_BITS = (128, 160, 192, 224, 256)
WORDLIST_SIZE = 2048


class MnemonicResult(TypedDict):
    ok: bool
    mnemonic: str
    word_count: int
    strength: int
...
def _generate_entropy(bits: int) -> bytes:
    if bits not in ENTROPY_BITS:
        raise ValueError(f"entropy bits must be one of {ENTROPY_BITS}")
    return secrets.token_bytes(bits // 8)


def _compute_checksum(entropy: bytes) -> str:
    hs = hashlib.sha256(entropy).digest()
    return bin(int.from_bytes(hs, "big"))[2:].zfill(256)[: len(entropy) // 4]


def _entropy_to_mnemonic(entropy: bytes) -> tuple[list[str], str]:
    if len(entropy) not in (16, 20, 24, 28, 32):
        raise ValueError("entropy must be 16, 20, 24, 28, or 32 bytes")

    checksum = _compute_checksum(entropy)
    ...
    for i in range(0, len(bits), 11):
        ...
        if idx >= WORDLIST_SIZE:
            raise ValueError(f"wordlist index {idx} out of range (max {WORDLIST_SIZE - 1})")
...
def validate_mnemonic(mnemonic: str) -> bool:
    ...
    if len(words) not in (12, 15, 18, 21, 24):
        return False
    for w in words:
        if w not in WORDLIST_INDEX:
            return False
    return True


def handler(event: dict) -> MnemonicResult | ErrorResult:
    """
    Generate a BIP-39 mnemonic phrase.

    Args:
        event: Dict with optional 'strength' (128, 160, 192, 224, 256 bits)
               and optional 'entropy' (hex string for deterministic generation)

    Returns:
        MnemonicResult on success or ErrorResult on failure.
    """
    if not isinstance(event, dict):
        return ErrorResult(ok=False, error="event must be an object")

    try:
        strength = int(event.get("strength", 128))
        if strength not in ENTROPY_BITS:
            return ErrorResult(
                ok=False,
                error=f"strength must be one of {ENTROPY_BITS}",
            )
    except (ValueError, TypeError):
        return ErrorResult(ok=False, error="strength must be an integer")

    entropy_hex = event.get("entropy")
    try:
        if entropy_hex and isinstance(entropy_hex, str):
            entropy = bytes.fromhex(entropy_hex)
            expected_len = strength // 8
            if len(entropy) != expected_len:
                return ErrorResult(
                    ok=False,
                    error=f"entropy must be {expected_len} bytes ({strength} bits), got {len(entropy)}",
                )
        else:
            entropy = _generate_entropy(strength)
    except Exception as e:
        return ErrorResult(ok=False, error=f"invalid entropy: {e}")

    try:
        words, _ = _entropy_to_mnemonic(entropy)
        mnemonic = " ".join(words)
        ...
        logger.error("mnemonic generation failed: %s", e)
        return ErrorResult(ok=False, error=str(e))
