...
def get_previous_runs(expression: str, count: int = 5, from_time: Optional[datetime] = None) -> List[str]:
    """Calculate previous run times for cron expression"""
    ...
    while len(previous_runs) < count:
        ...
        if (current.minute in minute_vals and
            ...
            previous_runs.append(current.isoformat() + 'Z')
...
def handler(event):
    try:
        ...
        previous_runs = get_previous_runs(expression, count, from_time)
        return {"ok": True, "previous_runs": previous_runs}
