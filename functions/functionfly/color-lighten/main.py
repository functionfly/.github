...
def _rgb_hsl(r,g,b):
    r_,g_,b_=r/255,g/255,b/255; cmax,cmin=max(r_,g_,b_),min(r_,g_,b_); delta=cmax-cmin
...
def handler(event):
    ...
    amount = float(event.get("amount", 10))
    if not color:
        return {"ok": False, "error": "color is required"}
    try:
        r,g,b=_parse(str(color)); h,s,l=_rgb_hsl(r,g,b)
        l2=min(100, l+amount)
        nr,ng,nb=_hsl_rgb(h,s,l2)
        return {"ok": True, "result": f"#{nr:02X}{ng:02X}{nb:02X}", "original_lightness": round(l,2), "new_lightness": round(l2,2)}
