...
export interface MetricCardProps extends TelemetryMetric {
  size?: "sm" | "md" | "lg";
  sparkline?: number[];
  onMetricClick?: (metric: TelemetryMetric) => void;
}
...
function getStatusColor(status: string): string {
  return { ok: "#10b981", warning: "#f59e0b", error: "#ef4444" }[status] || "#6b7280";
}
...
export function MetricCard({
  ...
  const sizeClasses = { sm: "p-3 min-h-[80px]", md: "p-4 min-h-[100px]", lg: "p-5 min-h-[120px]" };
  return (
    <div
      className={cn(
      ...
      onClick={() => onMetricClick?.({ label, value, unit, timestamp: Date.now(), id: "", status } as TelemetryMetric)}
    >
      <div className="flex items-center justify-between mb-1">
        <span className="text-[11px] text-text-muted capitalize">{label}</span>
        <div className="flex items-center gap-1">
          {getTrendIcon(trend)}
          {delta != null && <span className={`text-[10px] font-medium ${delta >= 0 ? "text-error" : "text-success"}`}>{formatDelta(delta)}</span>}
          <div className={`size-2 rounded-full ${status === "error" ? "bg-error" : status === "warning" ? "bg-warning" : "bg-success"}`} />
        </div>
      </div>
        ...
        <div className="mt-2 h-6 flex items-end gap-0.5">
          {sparkline.map((v, i) => (
            <div key={i} className="flex-1 rounded-t-[1px] transition-all duration-300"
              style={{ height: `${Math.max(v * 100, 4)}%`, backgroundColor: getStatusColor(status), opacity: 0.4 + v * 0.6 }} />
          ))}
        </div>
      )}
    </div>
    ...
    <div className={cn("space-y-4", className)}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-text-primary">Live Telemetry</span>
          ...
          {timeRanges.map((r) => (
            <button key={r} onClick={() => onTimeRangeChange?.(r)}
              className={cn("px-2 py-0.5 text-[10px] rounded transition-colors",
                timeRange === r ? "bg-brand-500/20 text-brand-400 font-medium" : "text-text-muted hover:text-text-secondary")}
            >{r}</button>
          ))}
        </div>
      </div>
      {showAlerts && metrics.some((m) => m.status === "error") && (
        <div className="flex items-center gap-2 px-3 py-2 bg-error/10 border border-error/30 rounded-lg text-sm text-error">
          ...
          <span>{metrics.filter((m) => m.status === "error").length} metrics in error state</span>
        </div>
      )}
      <div className="grid grid-cols-2 gap-3">
        <MetricCard label="Requests (24h)" value={12453} unit="req" sparkline={[0.3, 0.5, 0.8, 0.6, 0.9, 1, 0.7, 0.85, 0.6, 0.95]} size="md" onMetricClick={onMetricClick} />
        <MetricCard label="Avg Latency" value={42} unit="ms" status="ok" sparkline={[0.4, 0.3, 0.6, 0.5, 0.4, 0.7, 0.3, 0.5, 0.6, 0.4]} size="md" onMetricClick={onMetricClick} />
        <MetricCard label="Error Rate" value={0.2} unit="%" status="ok" sparkline={[0.1, 0.2, 0.1, 0.3, 0.2, 0.1, 0.4, 0.2, 0.1, 0.2]} size="md" onMetricClick={onMetricClick} />
        <MetricCard label="Active Agents" value={5} unit="agents" status="ok" sparkline={[0.2, 0.3, 0.5, 0.4, 0.6, 0.8, 0.7, 0.9, 0.8, 1]} size="md" onMetricClick={onMetricClick} />
      </div>
      {showTokenUsage && tokenUsage && <TokenUsageStream usage={tokenUsage} onTimeRangeChange={onTimeRangeChange} />}
    </div>
  );
}

export function TokenUsageStream({ usage, onTimeRangeChange, className }: TokenUsageStreamProps) {
  const totalCost = Object.values(usage.models).reduce((sum, m) => sum + m.cost, 0);
  return (
    <div className={cn("bg-bg-secondary rounded-lg border border-border-subtle p-4", className)}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {Object.keys(usage.models).map((model) => (
            <button key={model} onClick={() => onTimeRangeChange?.("24h")}
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="bg-bg-primary rounded-lg p-3 min-w-0">
          ...
          <div className="text-lg font-bold text-text-primary">{usage.totalTokens.toLocaleString()}</div>
          <div className="text-[10px] text-text-muted mt-1">In: {usage.inputTokens.toLocaleString()} · Out: {usage.outputTokens.toLocaleString()}</div>
        </div>
        <div className="bg-bg-primary rounded-lg p-3 min-w-0">
          <div className="text-[10px] text-text-muted mb-1">Total Cost</div>
          <div className="text-lg font-bold text-brand-500">${totalCost.toFixed(4)}</div>
          <div className="text-[10px] text-text-muted mt-1">{usage.totalTokens > 0 ? `$${(totalCost / usage.totalTokens * 1000).toFixed(4)}/1K tokens` : "$0.0000/1K tokens"}</div>
        </div>
        <div className="bg-bg-primary rounded-lg p-3 min-w-0">
          <div className="text-[10px] text-text-muted mb-1">Total Calls</div>
          <div className="text-lg font-bold text-text-primary">{Object.values(usage.models).reduce((s, m) => s + m.calls, 0).toLocaleString()}</div>
          <div className="text-[10px] text-text-muted mt-1">Models: {Object.keys(usage.models).length}</div>
        </div>
        <div className="bg-bg-primary rounded-lg p-3 min-w-0">
          <div className="text-[10px] text-text-muted mb-1">Cost Trend</div>
          <div className="text-lg font-bold text-success">↓ 12%</div>
          <div className="text-[10px] text-text-muted mt-1">vs. last 7 days</div>
        </div>
      </div>
      <div className="h-2 bg-bg-primary rounded-full overflow-hidden">
        {Object.entries(usage.models).map(([model, data], i, arr) => (
          <div key={model} className="h-full transition-all duration-500"
            style={{ width: usage.totalTokens > 0 ? `${(data.tokens / usage.totalTokens) * 100}%` : "0%", backgroundColor: `hsl(${i * 137.5}, 70%, 55%)` }} />
        ))}
      </div>
...
export function CostHeatmap({ data, className }: CostHeatmapProps) {
  ...
  return (
    <div className={cn("bg-bg-secondary border border-border-subtle rounded-lg p-4", className)}>
      <h4 className="text-sm font-medium text-text-primary mb-3">Cost Heatmap (Hours × Days)</h4>
      <div className="overflow-x-auto -mx-4 px-4">
        <div className="grid gap-[1px]" style={{ gridTemplateColumns: 'repeat(24, minmax(16px, 1fr))', minWidth: 400 }}>
          {Array.from({ length: 24 }, (_, h) => (
            <React.Fragment key={h}>
              <div className="text-[9px] text-text-muted text-center py-0.5 font-mono">{String(h).padStart(2, "0")}</div>
              {data.filter((d) => d.hour === h).map((d, i) => (
                <div key={i} className="aspect-square rounded-[1px]"
                  style={{ backgroundColor: `rgba(249, 115, 22, ${0.1 + (d.cost / maxCost) * 0.9})` }}
      ...
      <div className="flex items-center justify-between mt-2 text-[10px] text-text-muted">
        ...
        <div className="flex gap-0.5">
          {[0.1, 0.3, 0.5, 0.7, 0.9, 1].map((v) => (
            <div key={v} className="size-3 rounded-[1px]" style={{ backgroundColor: `rgba(249, 115, 22, ${v})` }} />
          ))}
        </div>
        <span>High cost</span>
      </div>
...
export function LatencyGraph({ data, timeRange, height = 120, className }: LatencyGraphProps) {
  ...
  const polyline = (key: "p50" | "p95" | "p99", color: string) => {
    ...
    return (
      <polyline key={key} points={points.join(" ")} fill="none" stroke={color}
  ...
  return (
    <div className={className}>
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-[10px] text-text-muted">p99: {data[data.length - 1]?.p99 ?? 0}ms</span>
      </div>
      <svg width="100%" height={height} viewBox={`0 0 100 ${height}`} preserveAspectRatio="none"
        ...
        {[0.25, 0.5, 0.75].map((y) => (
          <line key={y} x1="0" y1={y * height} x2="100" y2={y * height}
            stroke="rgba(255,255,255,0.04)" strokeWidth="0.5" />
        ))}
        {data.length > 1 && (
          <polygon
            points={(() => {
              const pts = data.map((d, i) => `${(i / (data.length - 1)) * 100},${100 - ((d.p50 || 0) / maxP99) * 100}`);
      ...
      <div className="flex items-center justify-between mt-1.5 text-[10px] text-text-muted">
        ...
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-[#3b82f6]" />p50</span>
          <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-[#f59e0b]" />p95</span>
          <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-[#ef4444]" />p99</span>
        </div>
...
export function ExecutionProfiler({ executions, className }: ExecutionProfilerProps) {
  const sorted = [...executions].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  const statusColor = { success: "#10b981", error: "#ef4444", timeout: "#f59e0b" };
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className="space-y-1">
        {sorted.slice(0, 10).map((exec) => (
          <div key={exec.id} className="flex items-center justify-between p-2.5 bg-bg-primary rounded-lg border border-border-subtle hover:border-border-default transition-colors">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="size-2 rounded-full shrink-0" style={{ backgroundColor: statusColor[exec.status] }} />
              <span className="text-sm font-medium text-text-primary truncate">{exec.name}</span>
              <span className="text-[10px] text-text-muted truncate">{exec.timestamp}</span>
            </div>
            <div className="flex items-center gap-4 shrink-0">
              <span className="text-[10px] text-text-muted font-mono">{exec.duration}ms</span>
              <span className="text-[10px] text-text-muted font-mono">${exec.cost.toFixed(4)}</span>
              <span className="text-[10px] text-text-muted">{exec.tokens.toLocaleString()} tok</span>
              {exec.retries > 0 && <Badge variant="warning" size="sm">{exec.retries} retry</Badge>}
            </div>
          </div>
        ))}
      </div>
...
function Badge({ variant = "default", size = "md", className, children }: any) {
  const sizeClasses = { sm: "px-1.5 py-0.5 text-[10px]", md: "px-2 py-0.5 text-[11px]" };
  const variantClasses = {
    default: "bg-bg-tertiary text-text-secondary", brand: "bg-brand-500/20 text-brand-400",
    success: "bg-success/20 text-success", error: "bg-error/20 text-error",
    warning: "bg-warning/20 text-warning", ghost: "bg-transparent text-text-muted",
  };
  return <span className={`inline-flex items-center gap-1 rounded-full font-medium ${sizeClasses[size]} ${variantClasses[variant]} ${className || ""}`}>{children}</span>;
}
// ============================================================================
...
export function InferenceTraceViewer({
  ...
  const [expandedSpans, setExpandedSpans] = React.useState<Set<string>>(new Set([spans[0]?.id]))
  ...
  const flattenSpans = (spanList: InferenceSpan[], depth = 0): Array<InferenceSpan & { depth: number }> => {
        ...
        {flattenedSpans.map((span) => {
          ...
          return (
            ...
            >
              ...
              >
                {hasChildren ? (expandedSpans.has(span.id) ? <ChevronDown className="size-3" /> : <ChevronRight className="size-3" />) : null}
              </button>

              {/* Status dot */}
              <div className={cn("size-2 rounded-full shrink-0", statusColors[span.status])} />
              ...
              <div className="flex-1 relative h-4">
                ...
                <div
                  className={cn("absolute inset-y-0 rounded transition-all", statusColors[span.status], span.status === "running" && "animate-pulse")}
                  style={{
                    left: "0%",
                    width: span.duration ? `${Math.min((span.duration / (spans[0]?.duration || 1)) * 100, 100)}%` : "20%"
                  }}
                />
              </div>
              ...
              <div className="w-16 text-right text-text-muted">
                {span.inputTokens && span.outputTokens ? `${span.inputTokens + span.outputTokens}` : "—"}
              </div>
            </div>
          )
        })}
...
export function ErrorCascadeVisualizer({
  ...
  const renderNode = (node: ErrorNode, depth = 0): JSX.Element => {
    ...
    return (
      <div key={node.id}>
        ...
        >
          <AlertTriangle className={cn("size-4 shrink-0", node.error ? "text-error" : "text-text-muted")} />
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-text-primary">{node.name}</div>
            {node.error && <div className="text-[11px] text-error mt-0.5 line-clamp-1">{node.error}</div>}
          </div>
          <span className="text-[10px] text-text-muted capitalize px-1.5 py-0.5 bg-bg-tertiary rounded">{node.type}</span>
        </button>
...
export function MemoryPressureMonitor({ data, className }: MemoryPressureMonitorProps) {
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="space-y-2">
        {data.map((runtime) => {
          ...
          return (
            <div key={runtime.runtime} className="p-3 bg-bg-secondary rounded-lg border border-border-subtle">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Server className="size-4 text-text-muted" />
                  <span className="text-sm font-medium text-text-primary">{runtime.runtime}</span>
                </div>
                <span className={cn("text-[10px] px-2 py-0.5 rounded-full capitalize", pressureColors[runtime.pressure])}>
                  {runtime.pressure}
                </span>
              </div>
              <div className="h-2 bg-bg-tertiary rounded-full overflow-hidden mb-1">
                <div
                  className={cn("h-full rounded-full transition-all", pressureBarColors[runtime.pressure])}
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {data.map((gpu) => {
          ...
          return (
            <div key={gpu.gpuId} className="p-4 bg-bg-secondary rounded-lg border border-border-subtle">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-text-muted">{gpu.gpuId}</div>
                  </div>
                </div>
                {gpu.temperatureC && (
                  <div className="flex items-center gap-1 text-[10px]">
                    <Thermometer className="size-3" />
                    <span className={gpu.temperatureC > 80 ? "text-error" : gpu.temperatureC > 60 ? "text-warning" : "text-text-muted"}>
                      {gpu.temperatureC}°C
                    </span>
                  </div>
                )}
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between text-[10px] mb-1">
                  ...
                  <span className="font-medium text-text-primary">{gpu.utilizationPct}%</span>
                </div>
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between text-[10px] mb-1">
                  ...
                  <span className="font-medium text-text-primary">{gpu.memoryUsedMB} / {gpu.memoryTotalMB} MB</span>
                </div>
                <div className="h-2 bg-bg-tertiary rounded-full overflow-hidden">
                  <div
              ...
              {gpu.powerW && (
                <div className="flex items-center justify-between text-[10px]">
                  ...
                  <span className="font-medium text-text-primary">{gpu.powerW}W</span>
                </div>
              )}
            </div>
          )
...
export function BandwidthUsagePanel({ data, className }: BandwidthUsagePanelProps) {
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-bg-secondary rounded-lg border border-border-subtle">
          ...
          <div className="text-lg font-bold text-success">↓ {totalRx.toFixed(2)} MB</div>
        </div>
        <div className="p-3 bg-bg-secondary rounded-lg border border-border-subtle">
          <div className="text-[10px] text-text-muted mb-1">Upload</div>
          <div className="text-lg font-bold text-brand-500">↑ {totalTx.toFixed(2)} MB</div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {data.map((iface) => (
          <div key={iface.interface} className="p-3 bg-bg-secondary rounded-lg border border-border-subtle">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <HardDrive className="size-4 text-text-muted" />
                <span className="text-sm font-medium text-text-primary">{iface.interface}</span>
              </div>
              <span className="text-[10px] text-text-muted">
                {iface.rxPackets.toLocaleString()} / {iface.txPackets.toLocaleString()} packets
              </span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <div className="text-[9px] text-text-muted mb-0.5">Download</div>
                <div className="h-1.5 bg-bg-tertiary rounded-full overflow-hidden">
                  <div className="h-full bg-success rounded-full" style={{ width: `${Math.min((iface.rxMB / Math.max(totalRx, 1)) * 100, 100)}%` }} />
                </div>
                <span className="text-[10px] text-text-primary">{iface.rxMB.toFixed(3)} MB</span>
              </div>
              <div>
                <div className="text-[9px] text-text-muted mb-0.5">Upload</div>
                <div className="h-1.5 bg-bg-tertiary rounded-full overflow-hidden">
                  <div className="h-full bg-brand-500 rounded-full" style={{ width: `${Math.min((iface.txMB / Math.max(totalTx, 1)) * 100, 100)}%` }} />
                </div>
                <span className="text-[10px] text-text-primary">{iface.txMB.toFixed(3)} MB</span>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className={cn("space-y-3", className)}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-text-muted font-mono">{traceId.slice(0, 12)}...</span>
      </div>
          ...
          <div key={service} className="space-y-0.5">
            ...
            {serviceSpans.map((span) => {
              ...
              return (
                ...
                >
                  <span className={cn("size-2 rounded-full shrink-0", span.status === "ok" ? "bg-success" : "bg-error")} />
                  <span className="w-32 truncate text-left text-text-primary">{span.operationName}</span>
                  <div className="flex-1 h-3 bg-bg-tertiary rounded relative">
                    <div
                      className={cn("absolute inset-y-0 rounded transition-all", span.status === "ok" ? "bg-brand-500" : "bg-error")}
                      style={{ left: "0%", width: `${(span.duration / maxDuration) * 100}%` }}
                    />
                  </div>
                  <span className="w-16 text-right text-text-muted font-mono">{span.duration.toFixed(1)}ms</span>
                </button>
              )
            })}
          </div>
...
export function RequestReplayPanel({ requests, onReplay, className }: RequestReplayPanelProps) {
  const [selectedRequest, setSelectedRequest] = React.useState<ReplayRequest | null>(null)
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isSelected ? "bg-brand-500/20 border border-brand-500/30" : "bg-bg-secondary border border-transparent hover:border-border-default"
              )}
            >
              <span className="text-[10px] font-mono text-text-muted w-32 truncate">{req.timestamp}</span>
              <span className={cn("text-[10px] font-bold w-8", req.method === "GET" ? "text-success" : req.method === "POST" ? "text-brand-400" : "text-warning")}>
                {req.method}
              </span>
              <span className="flex-1 truncate text-[11px] text-text-primary">{req.path}</span>
              <span className={statusColor(req.status)}>{req.status}</span>
              <span className="text-[10px] text-text-muted w-16 text-right">{req.duration}ms</span>
            </button>
          )
        })}
      </div>
        ...
        {logs.map((log) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("shrink-0", levelColors[log.level])}>{levelIcons[log.level]}</span>
              {log.service && <span className="text-brand-400 shrink-0">[{log.service}]</span>}
...
export function RuntimeMetricsGrid({ runtimes, onRuntimeClick, className }: RuntimeMetricsGridProps) {
  const statusColors = {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {runtimes.map((runtime) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className={cn("size-2 rounded-full", statusDot[runtime.status])} />
                <span className="text-sm font-medium text-text-primary">{runtime.name}</span>
              </div>
              <span className="text-[10px] text-text-muted capitalize">{runtime.status}</span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-[10px]">
              <div>
                <span className="text-text-muted">Requests</span>
                <div className="text-sm font-bold text-text-primary">{runtime.metrics.requests.toLocaleString()}</div>
              </div>
              <div>
                <span className="text-text-muted">Error Rate</span>
                <div className={cn("text-sm font-bold", runtime.metrics.errorRate < 1 ? "text-success" : runtime.metrics.errorRate < 5 ? "text-warning" : "text-error")}>
                  {runtime.metrics.errorRate.toFixed(2)}%
                </div>
              </div>
              <div>
                <span className="text-text-muted">p50 Latency</span>
                <div className="text-sm font-bold text-text-primary">{runtime.metrics.latencyP50}ms</div>
              </div>
              <div>
                <span className="text-text-muted">p99 Latency</span>
                <div className="text-sm font-bold text-text-primary">{runtime.metrics.latencyP99}ms</div>
              </div>
              <div>
                <span className="text-text-muted">CPU</span>
                <div className="text-sm font-bold text-text-primary">{runtime.metrics.cpu.toFixed(0)}%</div>
              </div>
              <div>
                <span className="text-text-muted">Memory</span>
                <div className="text-sm font-bold text-text-primary">{runtime.metrics.memory.toFixed(0)}%</div>
              </div>
            </div>
          </button>
        ))}
...
export function ObservabilityRadar({ metrics, className }: ObservabilityRadarProps) {
  const size = 200
  const center = size / 2
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected ? "bg-brand-500/10 border-brand-500/30" : "bg-bg-secondary border-border-subtle hover:border-border-default"
                )}
              >
                {/* Severity dot */}
                <div className={cn("size-8 rounded-full flex items-center justify-center shrink-0 z-10", severityColors[incident.severity])}>
                  {incident.severity === "critical" ? <XCircle className="size-4" /> : <AlertTriangle className="size-4" />}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {incident.description && (
                    <p className="text-[11px] text-text-secondary line-clamp-2 mb-2">{incident.description}</p>
                  )}
                  <div className="flex items-center gap-3 text-[10px] text-text-muted">
                    <span>{new Date(incident.startTime).toLocaleString()}</span>
                    {incident.endTime && <span>→ {new Date(incident.endTime).toLocaleString()}</span>}
...
export function AnomalyDetectionPanel({ anomalies, onAnomalyClick, className }: AnomalyDetectionPanelProps) {
  const severityColors = {
          ...
          >
            <div className="flex items-start justify-between mb-1">
              <div>
                ...
                <span className="text-[10px] text-text-muted ml-2">#{anomaly.id.slice(0, 6)}</span>
              </div>
              <span className="text-[10px] text-text-muted">{new Date(anomaly.detectedAt).toLocaleString()}</span>
            </div>
            <p className="text-[11px] text-text-secondary mb-2">{anomaly.description}</p>
            <div className="flex items-center gap-4 text-[10px]">
              <span className="text-text-muted">
                Value: <span className="text-error font-medium">{anomaly.value.toFixed(2)}</span>
              </span>
              <span className="text-text-muted">
                Expected: {anomaly.expectedRange[0].toFixed(2)} - {anomaly.expectedRange[1].toFixed(2)}
              </span>
              <span className="text-text-muted">
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-bg-secondary rounded-lg border border-border-subtle">
          ...
          <div className="text-lg font-bold text-text-primary">${currentCost.toFixed(2)}</div>
        </div>
        <div className="p-3 bg-bg-secondary rounded-lg border border-border-subtle">
          <div className="text-[10px] text-text-muted mb-1">Predicted (30d)</div>
          ...
          {projections.map((proj, i) => {
            ...
            return (
              <div key={proj.period} className="flex-1 flex flex-col items-center gap-1">
                ...
                />
                <span className="text-[9px] text-text-muted">{proj.period}</span>
              </div>
            )
          })}
        </div>
...
export function ResourceForecastGraph({
  ...
  const allValues = data.flatMap((d) => [d.actual, d.predicted, d.lower, d.upper].filter((v) => v != null) as number[])
  const maxValue = Math.max(...allValues, 1)
  const minValue = Math.min(...allValues, 0)

  const getY = (value: number) => 100 - ((value - minValue) / (maxValue - minValue)) * 100

  const actualPoints = data.filter((d) => d.actual != null).map((d, i) => ({
    x: (i / (data.length - 1)) * 100,
    y: getY(d.actual!),
  }))

  const predictedPoints = data.filter((d) => d.predicted != null).map((d, i) => ({
    x: (i / (data.length - 1)) * 100,
    y: getY(d.predicted!),
  }))

  const upperBand = data.filter((d) => d.upper != null).map((d, i) => `${(i / (data.length - 1)) * 100},${getY(d.upper!)}`)
  const lowerBand = data.filter((d) => d.lower != null).map((d, i) => `${(i / (data.length - 1)) * 100},${getY(d.lower!)}`).reverse()

  const separatorIndex = data.findIndex((d) => d.predicted != null && d.actual == null)
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className="flex items-center justify-between text-[10px] text-text-muted">
        <span>Past</span>
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-success" /> Actual</span>
          <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-brand-500" /> Predicted</span>
        </div>
        <span>Future</span>
      </div>
    </div>
