      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-xs">
          <thead className="sticky top-0 bg-aviation-bg-secondary">
            <tr>
              <th className="px-4 py-2 text-left font-medium text-aviation-text-muted">Role</th>
              {permissions.slice(0, 8).map(p => (
                <th key={p.id} className="px-3 py-2 text-center font-medium text-aviation-text-muted truncate max-w-[80px]" title={p.name}>
            ...
            {roles.map(role => (
              <tr
                ...
                className={cn(
                  ...
                  selectedRoleId === role.id ? 'bg-aviation-cyan/10' : 'hover:bg-aviation-bg-secondary'
                )}
              ...
              >
                <td className="px-4 py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="font-medium text-aviation-text-primary">{role.name}</div>
                      {showUserCount && (
                        <div className="text-[10px] text-aviation-text-muted">{role.userCount} users</div>
                      )}
                    </div>
                  </div>
                </td>
                {permissions.slice(0, 8).map(p => {
                  ...
                  return (
                    <td key={p.id} className="px-3 py-2 text-center">
                      <button
                        onClick={(e) => { e.stopPropagation(); onPermissionToggle?.(role.id, p.id); }}
                        className={cn(
                          'w-6 h-6 rounded flex items-center justify-center transition-colors',
                          hasPermission ? 'bg-green-500/20 text-green-400' : 'bg-aviation-bg-instrument text-aviation-text-muted'
                        )}
                      >
    ...
    <div className={cn('flex flex-col h-full bg-aviation-bg-panel rounded-lg border border-aviation-border-panel overflow-hidden', className)}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-aviation-border-panel">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-aviation-text-muted">{sectors.reduce((sum, s) => sum + s.threats.length, 0)} threats</span>
        </div>
      </div>
      <div className="flex-1 overflow-auto p-4">
        ...
        <div className="mt-4 space-y-2">
          {sectors.flatMap(s => s.threats).slice(0, 5).map((threat) => (
            <div
              key={threat.id}
              className={cn(
                'flex items-center gap-3 px-3 py-2 rounded cursor-pointer',
                selectedThreatId === threat.id ? 'bg-aviation-cyan/10' : 'hover:bg-aviation-bg-secondary'
              )}
              onClick={() => onThreatSelect?.(threat)}
            >
              <div className={cn('w-2 h-2 rounded-full', getSeverityBg(threat.severity))} />
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium capitalize">{threat.type}</div>
                <div className="text-[10px] text-aviation-text-muted truncate">{threat.source} → {threat.target}</div>
              </div>
              <span className={cn('text-[10px] px-1.5 py-0.5 rounded capitalize', getSeverityColor(threat.severity), 'bg-current/10')}>
                {threat.severity}
              </span>
            </div>
          ))}
        ...
        {showLegend && (
          <div className="flex items-center justify-center gap-4 mt-4 text-xs text-aviation-text-muted">
            ...
            <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-green-500" /> Low</span>
          </div>
        )}
      </div>
    </div>
...
export const SecurityTimeline: React.FC<SecurityTimelineProps> = ({
  ...
  const getEventIcon = (type: string) => {
    switch (type) {
      ...
      case 'permission-change': return <BadgeCheck className="w-4 h-4" />;
      case 'resource-access': return <Eye className="w-4 h-4" />;
          ...
          <div className="space-y-1 p-2">
            {events.map((event) => {
              ...
              return (
                ...
                >
                  <div className={cn('relative z-10 flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center', getSeverityColor(event.severity))}>
                    {getEventIcon(event.type)}
                  </div>
                  <div className="flex-1 min-w-0 pt-1">
                    <div className="text-xs text-aviation-text-primary">{event.message}</div>
...
const Timeline: React.FC<{ className?: string }> = ({ className }) => <History className={className} />;
...
export const SandboxBoundaryViewer: React.FC<SandboxBoundaryViewerProps> = ({
  ...
  const getBoundaryIcon = (type: string) => {
    switch (type) {
      ...
      case 'vm': return <MonitorIcon className="w-4 h-4" />;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', getStatusColor(boundary.status))}>
                    {boundary.status}
                  </span>
                </div>
                <div className="space-y-2">
                  {boundary.boundaries.map((b) => (
                    <div key={b.id} className="flex items-center justify-between">
                      <span className="text-xs text-aviation-text-muted capitalize">{b.type}</span>
                      <span className={cn(
                        'px-1.5 py-0.5 rounded text-[10px]',
                        b.status === 'enforced' ? 'text-green-400 bg-green-500/20' :
                        b.status === 'relaxed' ? 'text-amber-400 bg-amber-500/20' : 'text-red-400 bg-red-500/20'
                      )}>
                        {b.status}
                      </span>
                    </div>
...
export const APIExposureAnalyzer: React.FC<APIExposureAnalyzerProps> = ({
  ...
  const getMethodColor = (method: string) => {
    switch (method) {
      ...
      case 'PUT': return 'text-amber-400 bg-amber-500/20';
  ...
  return (
    <div className={cn('flex flex-col h-full bg-aviation-bg-panel rounded-lg border border-aviation-border-panel overflow-hidden', className)}>
      <div className="flex items-center gap-2 px-4 py-3 border-b border-aviation-border-panel">
        ...
        <span className="text-xs text-aviation-text-muted ml-auto">{endpoints.length} endpoints</span>
      </div>
      <div className="flex-1 overflow-auto">
        {endpoints.map((endpoint) => {
          ...
          return (
            ...
            >
              <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-mono', getMethodColor(endpoint.method))}>
                {endpoint.method}
              </span>
              <code className="flex-1 text-xs font-mono truncate">{endpoint.path}</code>
              <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', getRiskColor(endpoint.risk), 'bg-current/10')}>
                {endpoint.risk}
              </span>
              <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', 'text-aviation-text-muted bg-aviation-bg-secondary')}>
                {endpoint.exposure}
              </span>
            </div>
          );
...
export const CredentialAccessMonitor: React.FC<CredentialAccessMonitorProps> = ({
  accesses,
  selectedAccessId,
  onAccessSelect,
  className,
      ...
      <div className="flex-1 overflow-auto">
        {accesses.map((access) => {
          ...
          return (
            ...
            >
              <div className={cn('w-2 h-2 rounded-full', access.success ? 'bg-green-500' : 'bg-red-500')} />
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium">{access.credentialName}</div>
                <div className="text-[10px] text-aviation-text-muted">
                  {access.accessedBy} · {access.accessType}
                </div>
              </div>
              <div className="text-right">
                <div className="text-xs text-aviation-text-muted">{formatTime(access.accessedAt)}</div>
                {access.ipAddress && <div className="text-[10px] text-aviation-text-dim">{access.ipAddress}</div>}
              </div>
            </div>
          );
        })}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={cn('text-[10px] capitalize', getStatusColor(isolate.status))}>{isolate.status}</span>
                </div>
                <div className="flex items-center gap-4 text-[10px] text-aviation-text-muted">
                  <span className="capitalize">{isolate.type.replace('-', ' ')}</span>
                  {isolate.memoryUsage && <span>{isolate.memoryUsage}% memory</span>}
                  <span className={isolate.networkAllowed ? 'text-green-400' : 'text-red-400'}>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={cn('px-1.5 py-0.5 rounded text-[10px]', getStatusColor(fw.status))}>{fw.status}</span>
                </div>
                <div className="relative w-full h-2 bg-aviation-bg-secondary rounded-full overflow-hidden mb-2">
                  <div className="h-full bg-aviation-cyan transition-all" style={{ width: `${fw.score}%` }} />
                </div>
                ...
                <div className="mt-2 pt-2 border-t border-aviation-border-panel">
                  <div className="text-[10px] text-aviation-text-muted">{fw.requirements.length} requirements</div>
                </div>
              </div>
            );
          })}
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                flow.status === 'allowed' ? 'bg-green-500' :
                flow.status === 'denied' ? 'bg-red-500' : 'bg-amber-500'
              )} />
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium truncate">{flow.source.identity}</div>
                <div className="text-[10px] text-aviation-text-muted truncate">{flow.destination.identity}</div>
              </div>
              <div className="text-right">
                <div className="text-[10px] text-aviation-text-muted capitalize">{flow.status}</div>
                <div className="text-[10px] text-aviation-text-dim">{Math.round(flow.source.trustLevel * 100)}% trust</div>
              </div>
            </div>
          );
        })}
...
export const MaliciousExecutionDetector: React.FC<MaliciousExecutionDetectorProps> = ({
  executions,
  selectedExecutionId,
  onExecutionSelect,
  onBlock,
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isSelected ? 'bg-red-500/10' : 'hover:bg-aviation-bg-secondary cursor-pointer'
              )}
              onClick={() => onExecutionSelect?.(exec)}
            >
              <div className={cn('p-2 rounded', getSeverityColor(exec.severity))}>
                <Bug className="w-4 h-4" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-medium">{exec.processName}</div>
                <div className="text-[10px] text-aviation-text-muted">{exec.threatType} · {Math.round(exec.confidence * 100)}% confidence</div>
              </div>
              <div className="flex items-center gap-2">
                <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', getSeverityColor(exec.severity))}>
                  {exec.severity}
                </span>
                <button
                  onClick={(e) => { e.stopPropagation(); onBlock?.(exec.id); }}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] text-aviation-text-muted">{formatTime(entry.timestamp)}</span>
              </div>
              <div className="text-[10px] text-aviation-text-muted">
...
export const EncryptionStatusPanel: React.FC<EncryptionStatusPanelProps> = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'encrypted': return 'text-green-400 bg-green-500/20';
      case 'decrypted': return 'text-amber-400 bg-amber-500/20';
      case 'key-missing': return 'text-red-400 bg-red-500/20';
      default: return 'text-gray-400 bg-gray-500/20';
    }
  };
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', getStatusColor(status.status))}>
                    {status.status}
                  </span>
                </div>
                <div className="text-[10px] text-aviation-text-muted capitalize">{status.type.replace('-', ' ')}</div>
                {status.algorithm && <div className="text-[10px] text-aviation-text-dim mt-1">{status.algorithm}</div>}
              </div>
            );
          })}
        </div>
...
export const SuspiciousBehaviorTimeline: React.FC<SuspiciousBehaviorTimelineProps> = ({
  behaviors,
  selectedBehaviorId,
  onBehaviorSelect,
  className,
        ...
        {behaviors.map((behavior) => {
          ...
          return (
            <div
              ...
              className={cn(
                'px-4 py-3 border-b border-aviation-border-panel cursor-pointer',
                isSelected ? 'bg-amber-500/10' : 'hover:bg-aviation-bg-secondary'
              )}
              onClick={() => onBehaviorSelect?.(behavior)}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium capitalize">{behavior.type}</span>
                <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', getSeverityColor(behavior.severity))}>
                  {behavior.severity}
                </span>
              </div>
              <div className="text-xs text-aviation-text-primary mb-1">{behavior.description}</div>
  ...
  return (
    <div className={cn('flex flex-col h-full bg-aviation-bg-panel rounded-lg border border-aviation-border-panel overflow-hidden', className)}>
      <div className="flex items-center gap-2 px-4 py-3 border-b border-aviation-border-panel">
        ...
        <span className="text-xs text-aviation-text-muted ml-auto">{vulnerabilities.length} found</span>
      </div>
      <div className="flex-1 overflow-auto">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', getSeverityColor(vuln.severity))}>
                  {vuln.severity}
                </span>
              </div>
              <div className="flex items-center justify-between">
                {vuln.cveId && <span className="text-[10px] font-mono text-aviation-cyan">{vuln.cveId}</span>}
                {vuln.cvssScore && <span className="text-[10px] text-aviation-text-muted">CVSS: {vuln.cvssScore}</span>}
                <span className={cn('px-1.5 py-0.5 rounded text-[10px] capitalize', 'text-aviation-text-muted bg-aviation-bg-secondary')}>
                  {vuln.status}
                </span>
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <div className="text-[10px] text-aviation-text-muted capitalize">{policy.type}</div>
                </div>
                <button
                  onClick={(e) => { e.stopPropagation(); onPolicyToggle?.(policy.id); }}
                  className={cn(
                    'p-2 rounded transition-colors',
                    policy.status === 'active' ? 'text-green-400 bg-green-500/20' : 'text-gray-400 bg-aviation-bg-secondary'
                  )}
                >
                  {policy.status === 'active' ? <ShieldCheck className="w-4 h-4" /> : <ShieldAlert className="w-4 h-4" />}
                </button>
              </div>
              <p className="text-xs text-aviation-text-muted mb-3">{policy.description}</p>
