...
import { Play, Pause, CheckCircle2, XCircle, Clock, Loader2, ChevronRight } from "lucide-react";
...
const statusConfig = {
  ...
  skipped: { icon: ChevronRight, color: "text-text-muted", bg: "bg-bg-tertiary" },
};
    ...
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-text-primary">Execution</span>
                ...
                >
                  ...
                  >
                    {step.status === "running" && config.animate ? (
                      ...
                      <Icon className={cn("size-4", step.status === "running" && "animate-pulse")} />
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {step.duration && (
                        <Badge variant="ghost" size="sm">{getStepDuration(step)}</Badge>
                      )}
                      {isCurrent && step.status === "running" && (
                        <Badge variant="brand" size="sm" pulse>Running</Badge>
                      )}
                    </div>

                    {step.description && (
                      <p className="text-xs text-text-muted mb-2">{step.description}</p>
                    )}
