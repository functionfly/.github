...
CREATE INDEX idx_tenants_mfa_policy ON tenants(mfa_policy) WHERE mfa_policy IS NOT NULL;

-- Create index for faster user MFA enforcement lookups
CREATE INDEX idx_users_mfa_enforced ON users(mfa_enforced) WHERE mfa_enforced = true;
