-- Migration: Add capabilities field to functions
-- Date: 2024-02-20
-- Description: Adds capabilities column for sandbox permission enforcement

-- Create functions table (if not exists)
...
-- Backfill schema for deployments where functions table already exists without this column.
ALTER TABLE functions ADD COLUMN IF NOT EXISTS capabilities TEXT[] DEFAULT '{}';
...
CREATE INDEX IF NOT EXISTS idx_functions_capabilities ON functions USING GIN(capabilities);
