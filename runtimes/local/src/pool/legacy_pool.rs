//! Legacy metadata-only instance pool with memory optimization.
//!
//! This pool provides metadata-only tracking for memory pressure, LRU eviction,
//! and monitoring. It does not store actual WASM instances.
//!
//! For true warm-instance reuse, use `WasmInstancePool` instead.
...
use super::instance::PooledInstance;
use super::stats::PoolStats;

/// Pool of warm Wasm instances with memory optimization (metadata-only).
#[derive(Default)]
...
impl InstancePool {
    ...
    pub fn create_test_instance(instance_id: &str, memory_usage: usize) -> PooledInstance {
        PooledInstance::create_test(instance_id, memory_usage)
    }
}
